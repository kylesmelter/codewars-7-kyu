...
const draw = (deck) => {
    ...
    // Being we're given an empty array to start with, we don't need to create one
    // to push our cards to
    const drawnCards = [];
    // We'll create a while-loop because we aren't sure how many cards are within
    // the input deck, and we'll run it for as long as the deck contains 3 or more
    // cards. If there are less than 3, we can simply use the .push() method on the
    // remaining 2. During each loop we'll use the .push() method to add the value
    // of deck.shift() (the first value in the array) to our 'drawnCards' array, and
    // then we'll use the same two methods to add the next card in line to the back
    // of 'deck'  
    while(deck.length >= 3) {
        drawnCards.push(deck.shift());
        deck.push(deck.shift());
    };
    // Once the above while-loop has finished, we'll run another and use the .push()
    // method to add the remaining cards to our 'drawnCards' array
    while(deck.length >= 1) {
        drawnCards.push(deck.shift());
    };
    // All we need to do now is return our 'drawnCards' array!
    return drawnCards;
};