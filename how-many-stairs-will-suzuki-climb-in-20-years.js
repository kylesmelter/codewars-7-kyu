...
// stairs = [sunday,monday,tuesday,wednesday,thursday,friday,saturday]. Make sure your solution takes into account
// all of the nesting within the stairs array. Each weekday in the stairs array is an array.
...
function stairsIn20(s){
    // First, we'll create a new array and get rid of any and all nesting using .flat(Infinity)
    let array = s.flat(Infinity);
    // Now that any and all nesting is removed we can simply use the .reduce() method to sum all the numbers and 
    // multiply that result by 20 (number of years)
    let result = array.reduce((acc,c) => acc + c, 0) * 20;
    // All we have left to do is return our result!
    return result;
};