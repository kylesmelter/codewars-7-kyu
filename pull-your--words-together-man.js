...
function sentencify(words) {
    // Let's start with grabbing the first word in the input array, words, and capitalizing 
    // the first letter using the .toUpperCase() method, and then adding the rest of the word
    // using the .slice() method beginning with the second letter of the word
    words[0] = words[0][0].toUpperCase() + words[0].slice(1);
    // Next, we'll take the last word in the array, and set it equal to itself plus a '.'
    words[words.length - 1] = words[words.length - 1] + '.';
    // All we need to do now is return a joined version of the input array, words, using
    // the .join() method
    return words.join(' ');
};